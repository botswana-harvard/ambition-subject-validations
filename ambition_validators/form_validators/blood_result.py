...
class BloodResultFormValidator(FormValidator):
    ...
    def clean(self):
        ...
        self.range_gauge(field='magnesium', cleaned_data=self.cleaned_data,
                         lower_bound=0.75, upper_bound=1.2)

        self.range_gauge(field='potassium', cleaned_data=self.cleaned_data,
                         lower_bound=3.6, upper_bound=5.2)

        self.range_gauge(field='sodium', cleaned_data=self.cleaned_data,
                         lower_bound=135, upper_bound=145)

        self.range_gauge(field='alt', cleaned_data=self.cleaned_data,
                         lower_bound=10, upper_bound=40)

        self.range_gauge(field='platelets', cleaned_data=self.cleaned_data,
                         lower_bound=150, upper_bound=450)

        self.range_gauge(field='absolute_neutrophil', cleaned_data=self.cleaned_data,
